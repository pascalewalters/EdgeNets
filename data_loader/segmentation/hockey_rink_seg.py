...
'''
Custom data loader for the hockey line images
'''

# dataset has two classes, "Line" and "Background". Background is everything except lines
HOCKEY_DATASET_CLASS_LIST = ['background', 'line']


class HockeyRinkSegmentationDataset(torch.utils.data.Dataset):

    def __init__(self, root, train=True, scale=(0.5, 1.0), crop_size=(513, 513), ignore_idx=255, debug_labels=True):
        super(HockeyRinkSegmentationDataset, self).__init__()
        ...
        self.train = train
        if self.train:
            data_file = os.path.join(root, 'train_rink_seg.txt')
        else:
            data_file = os.path.join(root, 'val_rink_seg.txt')
        ...
        with open(data_file, 'r') as lines:
            for line in lines:
                ...
                rgb_img_loc = rgb_root_dir + os.sep + os.path.basename(line_split[0].strip())
                label_img_loc = label_root_dir + os.sep + os.path.basename(line_split[1].strip())
                assert os.path.isfile(rgb_img_loc), rgb_img_loc
                assert os.path.isfile(label_img_loc)
                self.images.append(rgb_img_loc)

                if debug_labels:
                    # let us also check that label file only contain labels as listed in HOCKEY_DATASET_CLASS_LIST
                    ...
                    assert 0 <= min_val <= max_val < len(
                        HOCKEY_DATASET_CLASS_LIST), "Label image at {} has following values. " \
                                                    "However, it should contain values only between 0 " \
                                                    "and {}".format(unique_values.tolist(),
                                                                    len(HOCKEY_DATASET_CLASS_LIST) - 1)
