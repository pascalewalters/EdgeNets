...
class CBR(nn.Module):
    ...
    # def __init__(self, nIn, nOut, kSize, stride=1, dilation=1, groups=1, act_name='selu'):
        '''
        ...
        self.cbr = nn.Sequential(
            MaskedConv2d(nIn, nOut, kSize, stride=stride, padding=padding, bias=False, groups=groups, dilation=dilation),
            nn.BatchNorm2d(nOut),
...
class CB(nn.Module):
    ...
    def __init__(self, nIn, nOut, kSize, stride=1, dilation=1, groups=1):
        ...
        self.cb = nn.Sequential(
            MaskedConv2d(nIn, nOut, kSize, stride=stride, padding=padding, bias=False, groups=groups, dilation=1),
            nn.BatchNorm2d(nOut),
        )
...
class BR(nn.Module):
    ...
    # def __init__(self, nOut, act_name='selu'):
        '''
        :param nIn: number of input channels
        :param act_name: Name of the activation function
        '''
...
class DWConv(nn.Module):
    def __init__(self, nin):
        ...
        self.dw_layer = nn.Sequential(
            MaskedConv2d(nin, nin, kernel_size=3, stride=1, padding=1, bias=False, groups=nin),
            nn.BatchNorm2d(nin),
            nn.PReLU(nin)
        )
