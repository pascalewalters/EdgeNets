...
from nn_layers.prune_cnn_utils import CBR, BR, Shuffle
...
class EfficientPyrPool(nn.Module):
    ...
    def __init__(self, in_planes, proj_planes, out_planes, scales=[2.0, 1.5, 1.0, 0.5, 0.1], last_layer_br=True):
        ...
        # self.projection_layer = CBR(in_planes, proj_planes, 1, 1, act_name='selu')
        self.projection_layer = CBR(in_planes, proj_planes, 1, 1)
        for _ in enumerate(scales):
            self.stages.append(MaskedConv2d(proj_planes, proj_planes, kernel_size=3, stride=1, padding=1, bias=False, groups=proj_planes))

        self.merge_layer = nn.Sequential(
            # perform one big batch normalization instead of p small ones
            # BR(proj_planes * len(scales), act_name='selu'),
            BR(proj_planes * len(scales)),
            Shuffle(groups=len(scales)),
            # CBR(proj_planes * len(scales), proj_planes, 3, 1, groups=proj_planes, act_name='selu'),
            CBR(proj_planes * len(scales), proj_planes, 3, 1, groups=proj_planes),
            MaskedConv2d(proj_planes, out_planes, kernel_size=1, stride=1, bias=not last_layer_br),
        )
        if last_layer_br:
            # self.br = BR(out_planes, act_name='selu')
