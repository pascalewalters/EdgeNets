...
import math
from nn_layers.prune_cnn_utils import CBR
...
class EfficientPWConv(nn.Module):
    def __init__(self, nin, nout):
        ...
        self.wt_layer = nn.Sequential(
                        ...
                        MaskedConv2d(nin, nout, kernel_size=1, stride=1, padding=0, groups=1, bias=False),
                        nn.Sigmoid()
                    )

        self.groups = math.gcd(nin, nout)
        # self.expansion_layer = CBR(nin, nout, kSize=3, stride=1, groups=self.groups, act_name='selu')
